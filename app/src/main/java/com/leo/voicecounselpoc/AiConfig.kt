...
object AiConfig {
    ...
    /**
     * 모델에게 주는 도구(함수 호출) 설정 (이슈 #17 S5-2).
     *
     * ⚠️ Firebase 문서상 Live API 의 도구 사용은 "coming soon"(공식 미지원)이다. SDK 17.17.0 에 코드는 있어
     * 실기기에서 동작을 확인한다. 동작하지 않거나 대화를 해치면 false 로 끄고 출력 전사 신호 문장(B안)만 쓴다.
     * [Audio.USE_DIRECT_AUDIO] 경로에서만 쓰인다 — SDK 경로는 도구 호출을 앱이 받지 못한다.
     *
     * 2026-09-15 상태: 도구를 선언해도 **연결은 성공**한다(태블릿). 모델이 실제로 도구를 호출하는지는 발화가 필요해
     * 아직 확인하지 못했다. 검증 전이라 기본값은 false — 스파이크할 때 true 로 바꾸고 "지금 몇 시 몇 분이에요?"라고
     * 말해 `도구 호출: get_local_time` 로그와 한국 시각 응답을 확인한다.
     */
    object Tools {
