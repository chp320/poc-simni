...
- ✅ 이슈 #24(세션이 대화 중 죽음) **2026-09-14 해결** — Spark 요금제가 원인
- ✅ S9 에코 캔슬 **1단계 완료** — S26 스피커 되먹임 해결 (#12, 끼어들기·AirPods·태블릿 검증 대기)
- ✅ **백그라운드 통화 유지 완료** — 화면 켜짐 유지(#32) + 포그라운드 서비스 B1(#13)
- **다음 작업 순서 (2026-09-14 사용자 결정)**
  1. ~~**going away 실측**~~ — ✅ 완료: 538초에 예고(timeLeft 50초, **중복 2회**), 589초에 실제 종료 (#34)
  2. ~~**#34 A안 구현**~~ — ✅ 완료: 태블릿 22분 32초 통화, 재연결 2회(1.5초), 15분 제한 통과. 맥락 유지·끼어들기는 S26 재테스트 대기
  3. ~~**S4 면책 고지**~~ — ✅ 완료 (2026-09-15, 전문가 검토만 남음)
  4. **다음: S5 위기 대응** (#17, 권장 순서) — 시작 전 `## ⚠️ 검증 대기` 재요청
- 전체 계획과 상태는 이슈 #21 (맨 위 "현재 상태" 표)
- **구현 항목·테스트 결과·발견한 문제는 모두 GitHub 이슈로 관리한다** — 새로 발견하면 이슈를 만들고 #21 에 연결

### 다음 세션에서 먼저 할 것
1. **`## ⚠️ 검증 대기` 확인** — 특히 **지시문 v3** 실대화 검증과 **통화 경로에서의 끼어들기**. 사용자에게 가능한지 먼저 묻는다 (스크립트 #25)
2. 검증이 끝났거나 보류되면 `### 현재 위치` 의 작업 순서대로 진행 (going away 실측 → #34 A안 → S4)
3. 테스트할 때: S26 스피커 사용 가능 (되먹임 해결). 이어폰은 통화 전에 연결
4. S26 은 삼성 **자동 차단(rampart)** 이 adb 를 수시로 끊는다 — 끊기면 자동 차단 끄기 → USB 디버깅 재활성화. 로그 버퍼를 16MB 로 늘려두면(`logcat -G 16M`) 끊긴 동안의 로그도 재연결 후 읽을 수 있다
...
├── LiveSessionManager.kt  Live 세션 연결/대화, 직접 수신·재개 핸들·going away 재연결·맥락 압축, PublicPreviewAPI 를 이 파일에 가둠
...
└── AiConfig.kt            모델명, 오디오 포맷·경로 스위치, 턴 감지, 통화 시간 정책
```
