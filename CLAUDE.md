...
- **🔴 진행 차단 중** — 이슈 #24: Live 세션이 대화 중 죽는다. `## 🔴 미해결 장애` 섹션 참조.
  **Blaze 요금제 전환이 다음 행동이며 사용자 작업이다.**
- 차단 해소 후 다음 단계: S4 면책 고지 (이슈 #18, 규모 S)
- 전체 계획과 상태는 이슈 #21

### 다음 세션에서 먼저 할 것
1. **`## ⚠️ 검증 대기` 확인** — 남아 있으면 사용자에게 검증 가능한지 먼저 묻는다
2. **Blaze 전환 여부 확인** — 됐으면 2~3분 대화로 이슈 #24 재현 테스트가 최우선
3. 차단이 풀리지 않았으면 이슈 #24 에 막히지 않는 작업을 한다 (S4 면책 고지는 통화 세션과 무관)
...
- [x] **조언 요구 시 단정하지 않는지** (S3 / 이슈 #16) — ✅ 2026-09-13 통과
  ...
  - ⚠️ **"사용자님"이라고 부르던 문제는 2026-09-13 수정 완료** — 지시문에서 "사용자"를 모두 제거하고 호칭 규칙을 추가했다. 실제 발화로 재확인은 아직 못 했다(이슈 #24 로 세션이 죽어서).
