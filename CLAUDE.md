...
- [ ] **짧은 답변 후 3초 정적 체감** (#22)
- [ ] **통화 경로에서 끼어들기** (#12) — S26 스피커, 모델이 길게 말할 때 끊기. AEC 가 내 목소리까지 약하게 만들지 않았는지
- [ ] **S26 + AirPods** (#12, #27) — 통화 프로필(SCO) 전환, AirPods 마이크 인식
- [ ] **태블릿 스피커 볼륨 10 이상에서 되먹임** (#12)
- [ ] **통화 볼륨이 최소에서도 큰 문제 수용 여부** (#12) — 사용자 답변 대기
...
- 2026-09-14: 통화 볼륨이 최소(1/15)에서도 큰 문제는 **미결정으로 남긴다** - 1단계(통화 경로) 부작용이다. 통화 볼륨은 0 으로 내려가지 않고 통화 경로 스피커 게인이 크다. 사용자에게 수용 여부를 물었으나 답을 받지 못했다 — 다음 테스트 때 다시 확인한다 (영향받는 항목: 이슈 #12)
